...
int main(){
	...
	for(int t = 0; t < num_testcases; ++t){
		...
		Graph g; 
		map<int, int> policePos; //maps vertex index to number of polices at this vertex
		map<int, int> photoPos;  //maps vertex index to number of photos at this vertex
		
		for (int i=0; i<numStations; i++){
			cin >> position;
			policePos[position]++;
			add_vertex(position+numIntersections, g); //for policeman
			add_vertex(position+numIntersections*3, g); //For police stations (safes)
		}
		...
		Graph subGraph; //keeps only the intersection and roads part
		for (int k=0; k<numEdges; k++){ //For the graph itself (intersections and roads)
